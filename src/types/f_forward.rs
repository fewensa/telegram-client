...
impl TGMessageForwardInfo {
  pub fn origin(&self) -> TGMessageForwardOrigin { self.td_origin().origin().map(|v| TGMessageForwardOrigin::of(v)).expect(&errors::data_fail_with_rtd(self.td_origin())[..]) }

  pub fn date(&self) -> i32 { self.td_origin().date().map(|v| v).expect(&errors::data_fail_with_rtd(self.td_origin())[..]) }

  pub fn from_chat_id(&self) -> i64 { self.td_origin().from_chat_id().map(|v| v).expect(&errors::data_fail_with_rtd(self.td_origin())[..]) }
...
impl TGMessageForwardOriginChannel {
  pub fn chat_id(&self) -> i64 { self.td_origin().chat_id().map(|v| v).expect(&errors::data_fail_with_rtd(self.td_origin())[..]) }

  pub fn message_id(&self) -> i64 { self.td_origin().message_id().map(|v| v).expect(&errors::data_fail_with_rtd(self.td_origin())[..]) }

  pub fn author_signature(&self) -> Option<String> { self.td_origin().author_signature() }
}

impl TGMessageForwardOriginHiddenUser {
  pub fn sender_name(&self) -> String { self.td_origin().sender_name().expect(&errors::data_fail_with_rtd(self.td_origin())[..]) }
}
