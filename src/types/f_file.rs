...
impl TGFile {
  ...
  pub fn size(&self) -> i32 { self.td_origin().size().expect(&errors::data_fail_with_rtd(self.td_origin())[..]) }

  pub fn expected_size(&self) -> Option<i32> { self.td_origin().expected_size() }

  pub fn local(&self) -> Option<TGLocalFile> { self.td_origin().local().map(|v| TGLocalFile::from_json(v.to_json()).expect(&errors::data_fail_with_rtd(self.td_origin())[..])) }

  pub fn remote(&self) -> Option<TGRemoteFile> { self.td_origin().remote().map(|v| TGRemoteFile::from_json(v.to_json()).expect(&errors::data_fail_with_rtd(self.td_origin())[..])) }
}
...
impl TGRemoteFile {
  ...
  pub fn uploaded_size(&self) -> i32 { self.td_origin().uploaded_size().expect(&errors::data_fail_with_rtd(self.td_origin())[..]) }
}
...
impl TGInputFileLocal {
  pub fn path(&self) -> String { self.td_origin().path().expect(&errors::data_fail_with_rtd(self.td_origin())[..]) }
}

impl TGInputFileRemote {
  pub fn id(&self) -> String { self.td_origin().id().expect(&errors::data_fail_with_rtd(self.td_origin())[..]) }
}

impl TGInputThumbnail {
  pub fn thumbnail(&self) -> TGInputFile { self.td_origin().thumbnail().map(|v| TGInputFile::of(v)).expect(&errors::data_fail_with_rtd(self.td_origin())[..]) }
