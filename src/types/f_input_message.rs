...
impl TGInputMessageAnimation {
  pub fn animation(&self) -> TGInputFile { self.origin().animation().map(|v| TGInputFile::of(v)).expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn thumbnail(&self) -> TGInputThumbnail {
    self.origin().thumbnail().map(|v| TGInputThumbnail::from_json(v.to_json()).expect(errors::TELEGRAM_DATA_FAIL))
      .expect(errors::TELEGRAM_DATA_FAIL)
  }

  pub fn duration(&self) -> Option<i32> { self.origin().duration() }
  ...
  pub fn caption(&self) -> Option<TGFormattedText> { self.origin().caption().map(|v| TGFormattedText::from_json(v.to_json()).expect(errors::TELEGRAM_DATA_FAIL)) }
}

impl TGInputMessageAudio {
  pub fn audio(&self) -> TGInputFile { self.origin().audio().map(|v| TGInputFile::of(v)).expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn album_cover_thumbnail(&self) -> TGInputThumbnail {
    self.origin().album_cover_thumbnail().map(|v| TGInputThumbnail::from_json(v.to_json()).expect(errors::TELEGRAM_DATA_FAIL))
      .expect(errors::TELEGRAM_DATA_FAIL)
  }

  pub fn duration(&self) -> Option<i32> { self.origin().duration() }

  pub fn title(&self) -> Option<String> { self.origin().title() }

  pub fn performer(&self) -> Option<String> { self.origin().performer() }

  pub fn caption(&self) -> Option<TGFormattedText> { self.origin().caption().map(|v| TGFormattedText::from_json(v.to_json()).expect(errors::TELEGRAM_DATA_FAIL)) }
}

impl TGInputMessageContact {
  pub fn contact(&self) -> Option<td_types::Contact> { self.origin().contact() }
}

impl TGInputMessageDocument {
  pub fn document(&self) -> TGInputFile { self.origin().document().map(|v| TGInputFile::of(v)).expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn thumbnail(&self) -> Option<TGInputThumbnail> { self.origin().thumbnail().map(|v| TGInputThumbnail::from_json(v.to_json()).expect(errors::TELEGRAM_DATA_FAIL)) }

  pub fn caption(&self) -> Option<TGFormattedText> { self.origin().caption().map(|v| TGFormattedText::from_json(v.to_json()).expect(errors::TELEGRAM_DATA_FAIL)) }
}

impl TGInputMessageForwarded {
  pub fn from_chat_id(&self) -> i64 { self.origin().from_chat_id().expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn message_id(&self) -> i64 { self.origin().message_id().expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn in_game_share(&self) -> bool { self.origin().in_game_share().map_or(false, |v| v) }
}

impl TGInputMessageGame {
  pub fn bot_user_id(&self) -> i32 { self.origin().bot_user_id().expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn game_short_name(&self) -> String { self.origin().game_short_name().expect(errors::TELEGRAM_DATA_FAIL) }
}

impl TGInputMessageInvoice {
  pub fn invoice(&self) -> TGInvoice { self.origin().invoice().map(|v| TGInvoice::from_json(v.to_json()).expect(errors::TELEGRAM_DATA_FAIL)).expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn title(&self) -> Option<String> { self.origin().title() }

  pub fn description(&self) -> Option<String> { self.origin().description() }

  pub fn photo_url(&self) -> Option<String> { self.origin().photo_url() }

  pub fn photo_size(&self) -> Option<i32> { self.origin().photo_size() }

  pub fn photo_width(&self) -> Option<i32> { self.origin().photo_height() }

  pub fn photo_height(&self) -> Option<i32> { self.origin().photo_height() }

  pub fn payload(&self) -> Option<String> { self.origin().payload() }

  pub fn provider_token(&self) -> Option<String> { self.origin().provider_token() }

  pub fn provider_data(&self) -> Option<String> { self.origin().provider_data() }

  pub fn start_parameter(&self) -> Option<String> { self.origin().start_parameter() }
}

impl TGInputMessageLocation {
  pub fn location(&self) -> td_types::Location { self.origin().location().expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn live_period(&self) -> Option<i32> { self.origin().live_period() }
}

impl TGInputMessagePhoto {
  pub fn photo(&self) -> TGInputFile { self.origin().photo().map(|v| TGInputFile::of(v)).expect(errors::TELEGRAM_DATA_FAIL) }
  ...
  pub fn added_sticker_file_ids(&self) -> Vec<i32> { self.origin().added_sticker_file_ids().map_or(vec![], |v| v) }
...
impl TGInputMessagePoll {
  pub fn question(&self) -> String { self.origin().question().expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn options(&self) -> Vec<String> { self.origin().options().map_or(vec![], |v| v) }
}

impl TGInputMessageSticker {
  pub fn sticker(&self) -> TGInputFile { self.origin().sticker().map(|v| TGInputFile::of(v)).expect(errors::TELEGRAM_DATA_FAIL) }
...
impl TGInputMessageText {
  pub fn text(&self) -> TGFormattedText { self.origin().text().map(|v| TGFormattedText::from_json(v.to_json()).expect(errors::TELEGRAM_DATA_FAIL)).expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn disable_web_page_preview(&self) -> bool { self.origin().disable_web_page_preview().map_or(false, |v| v) }

  pub fn clear_draft(&self) -> bool { self.origin().clear_draft().map_or(false, |v| v) }
}

impl TGInputMessageVenue {
  pub fn venue(&self) -> td_types::Venue { self.origin().venue().expect(errors::TELEGRAM_DATA_FAIL) }
}

impl TGInputMessageVideo {
  pub fn video(&self) -> TGInputFile { self.origin().video().map(|v| TGInputFile::of(v)).expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn thumbnail(&self) -> TGInputThumbnail { self.origin().thumbnail().map(|v| TGInputThumbnail::from_json(v.to_json()).expect(errors::TELEGRAM_DATA_FAIL)).expect(errors::TELEGRAM_DATA_FAIL) }
  ...
  pub fn supports_streaming(&self) -> bool { self.origin().supports_streaming().map_or(false, |v| v) }
...
impl TGInputMessageVideoNote {
  pub fn video_note(&self) -> TGInputFile { self.origin().video_note().map(|v| TGInputFile::of(v)).expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn thumbnail(&self) -> TGInputThumbnail { self.origin().thumbnail().map(|v| TGInputThumbnail::from_json(v.to_json()).expect(errors::TELEGRAM_DATA_FAIL)).expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn duration(&self) -> Option<i32> { self.origin().duration() }

  pub fn length(&self) -> i32 { self.origin().length().expect(errors::TELEGRAM_DATA_FAIL) }
}

impl TGInputMessageVoiceNote {
  pub fn voice_note(&self) -> TGInputFile { self.origin().voice_note().map(|v| TGInputFile::of(v)).expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn duration(&self) -> Option<i32> { self.origin().duration() }

  pub fn waveform(&self) -> Option<String> { self.origin().waveform() }

  pub fn caption(&self) -> Option<TGFormattedText> { self.origin().caption().map(|v| TGFormattedText::from_json(v.to_json()).expect(errors::TELEGRAM_DATA_FAIL)) }
}

impl TGFormattedText {
  pub fn text(&self) -> String { self.origin().text().expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn entities(&self) -> Vec<TGTextEntity> {
    self.origin().entities().map(|v| v.iter()
...
impl TGTextEntity {
  pub fn offset(&self) -> i32 { self.origin().offset().expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn length(&self) -> i32 { self.origin().length().expect(errors::TELEGRAM_DATA_FAIL) }

  pub fn type_(&self) -> TGTextEntityType { self.origin().type_().map(|v| TGTextEntityType::of(v)).expect(errors::TELEGRAM_DATA_FAIL) }
}
