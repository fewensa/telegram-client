...
pub struct RasyncListener {
  ...
  update_chat_chat_list: Option<Arc<dyn Send + Sync + Fn((Api, UpdateChatChatList)) -> LocalBoxFuture<'static, TGResult<()>>>>,
  ...
  update_chat_pinned_message: Option<Arc<dyn Send + Sync + Fn((Api, UpdateChatPinnedMessage)) -> LocalBoxFuture<'static, TGResult<()>>>>,
...
impl RasyncListener {
  ...
  pub fn on_update_chat_chat_list<F>(&mut self, fnc: F) -> &mut Self
    where F: Send + Sync + Fn((Api, UpdateChatChatList)) -> LocalBoxFuture<'static, TGResult<()>> + 'static {
  ...
  pub fn on_update_chat_pinned_message<F>(&mut self, fnc: F) -> &mut Self
    where F: Send + Sync + Fn((Api, UpdateChatPinnedMessage)) -> LocalBoxFuture<'static, TGResult<()>> + 'static {
...
impl RasyncLout {
  ...
  /// The list to which the chat belongs was changed. This update is guaranteed to be sent only when chat.order == 0 and the current or the new chat list is null
  pub fn update_chat_chat_list(&self) -> &Option<Arc<dyn Send + Sync + Fn((Api, UpdateChatChatList)) -> LocalBoxFuture<'static, TGResult<()>> + 'static>> {
  ...
  /// The chat pinned message was changed
  pub fn update_chat_pinned_message(&self) -> &Option<Arc<dyn Send + Sync + Fn((Api, UpdateChatPinnedMessage)) -> LocalBoxFuture<'static, TGResult<()>> + 'static>> {
